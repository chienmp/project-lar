							...
							<table class="table table-bordered" style="margin-top:20px;">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<img height="150px" src="{{asset('storage/avatar/'.$product->prod_img)}}" class="thumbnail">
										</td>
										<td>{{$product->cate_name}}</td>
										<td>
											<a href="{{asset('admin/product/edit/'.$product->prod_id)}}" class="btn btn-warning">
												<i class="fa fa-pencil" aria-hidden="true">Sửa</i></a>
											<a onclick="return confirm('Bạn chắc chắn muốn xóa ? ')" href="{{asset('admin/product/delete/'.$product->prod_id)}}" class="btn btn-danger">
